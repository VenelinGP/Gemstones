...
let User = require("../../models").User,
    fakeDb = require("../../models/fakeDatas"),
...
module.exports = {
    ...
    updateUserImage(userId, imageUrl) {
        return new Promise((resolve, reject) => {
            User.findOneAndUpdate({ "_id": userId }, { "image": imageUrl }, { save: true },
                (err, user) => {
                    if (err) {
                        return reject(err);
                    }
    ...
    addUnreadMessage(userId) {
        return new Promise((resolve, reject) => {
            User.findOneAndUpdate({ "_id": userId }, { $inc: { "unreadMessages": 1 } }, { save: true },
                (err, user) => {
                    if (err) {
                        return reject(err);
                    }
    ...
    restoreMessagesCount(userId) {
        return new Promise((resolve, reject) => {
            User.findOneAndUpdate({ "_id": userId }, { "unreadMessages": 0 }, { save: true },
                (err, user) => {
                    if (err) {
                        return reject(err);
                    }
